...
const SHEETS_WEBHOOK_URL =
  "https://script.google.com/macros/s/AKfycbyrfnrWvErokZhGmTRGw0k39h3l1nIxYCdBcr7XxlV4SygPrCRaZEQKMnNwYdjQTuQuKw/exec";
...
function getMRHomePrice(w) {
  return getMRRelayPrice(w) + MR_HOME_SURCHARGE;
}
...
const PRODUCTS = [
  ...
  {
    id: "feuilles_bissap",
    name: "Feuilles de bissap",
    emoji: "🍃",
    pricePerKg: 14,
    image: bissapFeuilleImg,
  },
  {
    id: "fleurs_bissap",
...
const PACKS = [
  ...
  {
    id: "pack_premium",
    name: "🔥 PACK PREMIUM DIASPORA",
    subtitle: "Pour les amoureux des saveurs authentiques",
    price: 69,
    isPremium: true,
    highlight: false,
    image: packDiasporaImg,
...
// ─── useCart ──────────────────────────────────────────────────────────────────
function useCart() {
  const [unitCart, setUnitCart] = useState(() =>
    Object.fromEntries(PRODUCTS.map((p) => [p.id, 0]))
  );
  const [packCart, setPackCart] = useState(() =>
    Object.fromEntries(PACKS.map((p) => [p.id, 0]))
  );
  ...
  const updateUnit = useCallback(
    (id, delta) =>
      setUnitCart((prev) => ({
        ...prev,
        [id]: Math.max(0, Math.round(((prev[id] || 0) + delta) * 10) / 10),
      })),
    []
  );
  ...
  const unitItems = useMemo(
    () =>
      PRODUCTS.filter((p) => unitCart[p.id] > 0).map((p) => ({
        ...p,
        qtyKg: unitCart[p.id],
        subtotal: unitCart[p.id] * p.pricePerKg,
      })),
    [unitCart]
  );

  const totalUnitWeight = useMemo(
    () => unitItems.reduce((s, i) => s + i.qtyKg, 0),
    [unitItems]
  );
  ...
  const unitItemsWithDiscount = useMemo(
    () =>
      unitItems.map((item) => ({
        ...item,
        discountedTotal: item.subtotal * (1 - discountRate),
      })),
  ...
  const selectedPacks = useMemo(
    () =>
      PACKS.filter((p) => packCart[p.id] > 0).map((p) => ({
        ...p,
        quantity: packCart[p.id],
      })),
    [packCart]
  );

  const unitSubtotal = useMemo(
    () => unitItemsWithDiscount.reduce((s, i) => s + i.discountedTotal, 0),
    [unitItemsWithDiscount]
  );
  const packsSubtotal = useMemo(
    () => selectedPacks.reduce((s, p) => s + p.price * p.quantity, 0),
    [selectedPacks]
  );
  ...
  const totalPackWeight = useMemo(
    () =>
      selectedPacks.reduce(
        (s, pk) => s + pk.items.reduce((a, b) => a + b.qtyKg, 0) * pk.quantity,
        0
      ),
    [selectedPacks]
  );
...
function buildWAMessage(cart, customer, fullPhone) {
  ...
  const packLines = selectedPacks.length
    ? selectedPacks
        .map(
          (p) =>
            `- ${p.name} ×${p.quantity} = ${(p.price * p.quantity).toFixed(
              2
            )} €`
        )
        .join("\n")
  ...
  const unitLines = unitItemsWithDiscount.length
    ? unitItemsWithDiscount
        .map(
          (i) =>
            `- ${i.emoji} ${i.name} : ${i.qtyKg
              .toFixed(1)
              .replace(".0", "")} kg × ${i.pricePerKg.toFixed(
              2
            )} €/kg = ${i.discountedTotal.toFixed(2)} €${
              bulkDiscount ? " (−15%)" : ""
            }`
        )
        .join("\n")
...
Livraison (${deliveryMode === "home" ? "Domicile" : "Point Relais"}) : ${
...
Téléphone : ${fullPhone}
Adresse : ${customer.adresse}, ${customer.codePostal} ${customer.ville}, ${
    customer.pays
  }`;
}
...
function Section({ id, bg, children, style = {} }) {
  return (
    <section
      id={id}
      style={{ background: bg || "transparent", padding: "28px 0", ...style }}
    >
  ...
  return (
    ...
    >
      <span style={{ fontSize: 13, fontWeight: "bold" }}>
        ⏱️ Commandez avant vendredi 23h59 pour une livraison cette semaine !
      </span>
      <div
        style={{
          display: "flex",
        ...
        {items.map((item, i) => (
          ...
          >
            ...
            <div>
              ...
              <div
                style={{ fontSize: 11, color: T.gray, whiteSpace: "nowrap" }}
              >
                {item.sub}
              </div>
            </div>
  ...
  return (
    ...
    >
      <span style={{ fontSize: 12, fontWeight: "bold", color }}>
        {info.stockLeft <= 3 ? "🔴" : "🟡"} Stock limité — {info.stockLeft} kg
        restants
      </span>
      <div
        style={{
...
function ValidatedInput({
  ...
  const borderColor =
    isValid === null ? "#cbd5e1" : isValid ? "#22c55e" : "#ef4444";
  return (
  ...
  return (
    <div style={{ display: "flex", gap: 8, gridColumn: "1 / -1" }}>
      ...
      <div style={{ position: "relative", flex: 1 }}>
        <input
          type="tel"
          placeholder="Numéro (sans le 0 initial)"
          value={phoneNumber}
...
function ProgressBar({ subtotal, relayIsFree, hasPremiumPack }) {
  ...
  const msg = relayIsFree
    ? hasPremiumPack
      ? "🎁 Livraison Relais offerte — Pack Premium !"
      : "🎁 Livraison Relais offerte !"
    : `Encore ${(FREE_DELIVERY_THRESHOLD - subtotal).toFixed(
        2
      )} € pour la livraison Relais gratuite`;
  return (
    <div style={{ marginBottom: 14 }}>
      <div
        style={{
          fontSize: 12,
          color: relayIsFree ? T.greenDark : T.gray,
          marginBottom: 5,
          fontWeight: "bold",
        }}
      >
      ...
      >
        <div
          style={{
            background: relayIsFree ? "#22c55e" : T.green,
            width: `${pct}%`,
            height: "100%",
            borderRadius: 999,
...
function BulkBanner({ totalUnitWeight, bulkDiscount }) {
  ...
  return bulkDiscount ? (
    ...
    >
      💡 Encore{" "}
      <strong>
        {(BULK_DISCOUNT_THRESHOLD - totalUnitWeight)
          .toFixed(1)
          .replace(".0", "")}{" "}
        kg
      </strong>{" "}
      pour obtenir <strong>−15%</strong> sur tous vos produits à l'unité !
    </div>
  );
}
        ...
        {[
          {
            ...
            label: "Point Relais",
            price:
              subtotal > 0 && relayIsFree ? (
                <span style={{ color: T.greenDark }}>Offerte 🎁</span>
              ) : (
                `${relayPrice.toFixed(2)} €`
              ),
            note: "3–5 j ouvrés",
          },
          {
            ...
            icon: "🏠",
            label: "À domicile",
            price: `${homePrice.toFixed(2)} €`,
            note: "Toujours payante",
          },
        ].map((opt) => (
          <button
            key={opt.key}
            ...
            style={{
              ...
              border:
                mode === opt.key
                  ? `2px solid ${T.green}`
                  : `1px solid ${T.border}`,
              background: mode === opt.key ? T.greenLight : T.white,
              transition: "all 0.2s",
            }}
          >
            <div
              style={{ fontWeight: "bold", fontSize: 13, color: T.greenDark }}
            >
              {opt.icon} {opt.label}
            </div>
            <div
              style={{
                fontSize: 11,
                color: opt.key === "home" ? "#9a3412" : T.gray,
                marginTop: 2,
              }}
            >
              {opt.note}
            </div>
            <div style={{ fontWeight: "bold", marginTop: 5, fontSize: 14 }}>
              {opt.price}
            </div>
          </button>
        ))}
  ...
  return (
    <div
      style={{
        ...card,
        position: "relative",
        border: hasStock ? "1.5px solid #fca5a5" : `1px solid ${T.border}`,
      }}
    >
      {hasStock && (
        <div
      ...
      <div style={{ color: T.gray, fontSize: 12, marginBottom: 8 }}>
        Unité : 0,5 kg
      </div>
      ...
      >
        <QtyControls
          value={qty > 0 ? `${qty.toFixed(1).replace(".0", "")} kg` : "0 kg"}
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div
            style={{
              fontWeight: "bold",
              color: bulkDiscount ? T.greenDark : "#1f2937",
              fontSize: 15,
            }}
          >
      ...
      <button type="button" onClick={onAdd} style={greenBtn}>
        + Ajouter 0,5 kg
      </button>
    </div>
  );
...
function PackCard({ pack, qty, onDecrement, onIncrement, onAdd }) {
  return (
    <div
      style={{
        ...card,
        border: pack.highlight ? "2px solid #22c55e" : `1px solid ${T.border}`,
        position: "relative",
    ...
    >
      ...
      >
        <span style={{ fontSize: 26, color: T.greenDark, fontWeight: "bold" }}>
      ...
      <p style={{ color: T.gray, fontSize: 13, margin: "0 0 14px" }}>
        {pack.description}
      </p>
      <div
        style={{
      ...
      >
        <QtyControls
          ...
          label={pack.name}
        />
        <button type="button" onClick={onAdd} style={{ ...greenBtn, flex: 1 }}>
          Ajouter au panier
        </button>
      </div>
    </div>
    ...
    <>
      ...
      {subtotal <= 0 ? (
        ...
        <>
          <ProgressBar
            subtotal={subtotal}
            relayIsFree={relayIsFree}
            hasPremiumPack={hasPremiumPack}
          />
          <BulkBanner
            totalUnitWeight={totalUnitWeight}
          ...
          {unitItemsWithDiscount.map((item) => (
            <div key={item.id} style={cartLine}>
              <span style={{ fontSize: 13 }}>
                ...
                {item.qtyKg.toFixed(1).replace(".0", "")} kg
              </span>
              <div style={{ textAlign: "right" }}>
                {bulkDiscount && (
                  <div
          ...
          <div style={{ ...cartLine, borderBottom: "none" }}>
            ...
            <span style={{ fontSize: 13 }}>
              {totalWeight.toFixed(1).replace(".0", "")} kg
            </span>
          </div>
          <div style={cartLine}>
            <span style={{ fontSize: 13 }}>Sous-total</span>
            <strong style={{ fontSize: 13 }}>{subtotal.toFixed(2)} €</strong>
          </div>
          ...
          <div style={{ ...cartLine, borderBottom: "none" }}>
            ...
            <strong
              style={{ color: deliveryCost === 0 ? T.greenDark : "#1f2937" }}
            >
              {deliveryCost === 0
                ? "Offerte 🎉"
          ...
          >
            <span>Total</span>
            <span style={{ color: T.greenDark }}>
              {grandTotal.toFixed(2)} €
            </span>
          </div>
        </>
      )}
      {subtotal > 0 && !relayIsFree && deliveryMode === "relay" && (
        <div style={warn}>
          Encore {savingsToFree.toFixed(2)} € pour la livraison Relais offerte.
        </div>
      )}
      {deliveryMode === "home" && subtotal > 0 && (
        <div
          style={{
            ...warn,
            background: "#eff6ff",
        ...
        >
          ℹ️ La livraison à domicile est toujours payante (
          {homePrice.toFixed(2)} €).
        </div>
      )}
      {missingFields && subtotal > 0 && (
        <div style={warn}>Remplissez tous les champs client pour valider.</div>
      )}
      {subtotal > 0 && !missingFields ? (
        <button
          ...
          style={{
            ...
            width: "100%",
            marginTop: 14,
            boxShadow: "0 6px 18px rgba(31,122,61,0.22)",
          }}
        >
          Voir le récapitulatif →
        </button>
      ) : (
        <button type="button" style={disBtn} disabled>
          Voir le récapitulatif →
        </button>
      )}
    </>
  ...
  const methods = [
      ...
      content: (
        <>
          ...
          <a
            ...
            onClick={() => onPaymentChosen("PayPal")}
          >
            <button
              type="button"
              style={{
      ...
      content: (
        <>
          <div style={{ fontSize: 13, color: T.gray, marginBottom: 12 }}>
            Ouvrez votre app Wero et envoyez <strong>{amount} €</strong> au
            numéro ci-dessous :
          </div>
          ...
          <button
            ...
            onClick={() => {
              copyWero();
              onPaymentChosen("Wero");
            }}
            style={{
              border: "2px solid #5b21b6",
              background: copied ? "#5b21b6" : T.white,
              color: copied ? T.white : "#5b21b6",
              borderRadius: 10,
              padding: "11px",
              fontWeight: "bold",
              cursor: "pointer",
          ...
          >
            ⚠️ Indiquez bien la référence <strong>La Serre Africaine</strong>{" "}
            lors de votre envoi Wero.
          </div>
        </>
      ),
    },
    {
      ...
      content: (
        ...
        >
          <div
            style={{ fontWeight: "bold", color: T.greenDark, marginBottom: 10 }}
          >
          ...
          {[
            ...
            >
              ...
              <strong
                style={{
                  fontFamily: l === "IBAN" ? "monospace" : undefined,
                  fontSize: l === "IBAN" ? 12 : undefined,
                  color: l === "Montant" ? T.greenDark : undefined,
                }}
              >
                {v}
              </strong>
          ...
          <button
            type="button"
            onClick={() => onPaymentChosen("Virement")}
            style={{
              border: "none",
              background: T.green,
              color: T.white,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2 style={{ margin: 0, color: T.greenDark, fontSize: 20 }}>
            💳 Choisir le paiement
          </h2>
          <button
            type="button"
        ...
        >
          <span
            style={{ color: T.greenDark, fontWeight: "bold", fontSize: 14 }}
          >
            Total à régler
          </span>
          <span
            style={{ color: T.greenDark, fontWeight: "bold", fontSize: 22 }}
          >
            {amount} €
          </span>
        </div>
          ...
          {methods.map((m) => (
            <div
              ...
              style={{
                ...
                padding: 16,
                cursor: "pointer",
                background: selected === m.id ? m.bg : T.white,
                transition: "all 0.2s",
              }}
            >
              <div
                style={{
                  ...
                  alignItems: "center",
                  gap: 10,
                  marginBottom: selected === m.id ? 14 : 0,
                }}
              >
                <span style={{ fontSize: 22 }}>{m.icon}</span>
                <div>
                  <div
                    style={{ fontWeight: "bold", color: m.color, fontSize: 15 }}
                  >
                    {m.label}
                  </div>
                ...
                <div
                  style={{
                    ...
                    borderRadius: "50%",
                    border: `2px solid ${
                      selected === m.id ? m.color : "#d1d5db"
                    }`,
                    background: selected === m.id ? m.color : T.white,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                  }}
        ...
        >
          <strong>Après paiement :</strong> votre commande sera confirmée par
          WhatsApp sous 24h. Conservez votre justificatif.
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ background: T.bg, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "24px 16px" }}>
        ...
        <div style={{ ...card, marginBottom: 20, textAlign: "center" }}>
          ...
          <h2 style={{ color: T.greenDark, margin: "0 0 8px", fontSize: 22 }}>
            Commande préparée !
          </h2>
          <p style={{ color: T.gray, fontSize: 14, margin: "0 0 16px" }}>
            Confirmez sur WhatsApp puis procédez au paiement.
          </p>
          {paymentMethod && (
            <div
              style={{
        ...
        <div style={{ ...card, marginBottom: 16 }}>
          <h3 style={{ color: T.greenDark, margin: "0 0 14px", fontSize: 17 }}>
            👤 Vos informations
          </h3>
          {[
            ["Nom", `${customer.prenom} ${customer.nom}`],
            ...
            [
              "Livraison",
              deliveryMode === "home" ? "À domicile" : "Point Relais",
            ],
          ].map(([l, v]) => (
            <div
              key={l}
        ...
        <div style={{ ...card, marginBottom: 16 }}>
          ...
          {unitItemsWithDiscount.map((i) => (
            <div key={i.id} style={cartLine}>
              <span style={{ fontSize: 14 }}>
                {i.emoji} {i.name} — {i.qtyKg.toFixed(1).replace(".0", "")} kg
              </span>
              <div style={{ textAlign: "right" }}>
                {bulkDiscount && (
                  <div
          ...
          <div style={{ ...cartLine, borderBottom: "none" }}>
            <span style={{ color: T.gray, fontSize: 13 }}>Poids total</span>
            <span style={{ fontSize: 13 }}>
              {totalWeight.toFixed(1).replace(".0", "")} kg
            </span>
          </div>
        ...
        <div style={{ ...card, marginBottom: 20 }}>
          <h3 style={{ color: T.greenDark, margin: "0 0 16px", fontSize: 17 }}>
            📋 Prochaines étapes
          </h3>
          {[
            {
              ...
              desc: "Votre commande est enregistrée automatiquement dans notre tableau de suivi.",
              color: T.greenLight,
            },
            {
              num: "2",
              title: "Régler votre commande",
              desc: "Choisissez Wero, PayPal ou virement bancaire.",
              color: "#e0f2fe",
            },
            {
              num: "3",
              title: "Expédition sous 48h",
              desc: "Votre colis est préparé et expédié via Mondial Relay.",
              color: T.amberLight,
            },
            {
              num: "4",
              title: "Livraison en 3–5 jours",
              desc: "Recevez vos légumes frais au Point Relais ou à domicile.",
              color: "#f3e8ff",
            },
          ].map((step) => (
            <div
            ...
            >
              ...
              <div>
                ...
                <div style={{ color: T.gray, fontSize: 13 }}>{step.desc}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          >
            ...
            {paymentMethod && (
              <span style={{ fontSize: 13, opacity: 0.85, marginLeft: 8 }}>
                — {paymentMethod}
              </span>
            )}
          </button>
...
function ProSection() {
  ...
  return (
    <div
      style={{
        background: "linear-gradient(135deg, #1c3d5a, #1f5c3d)",
        color: T.white,
        borderRadius: 18,
        padding: 28,
    ...
    >
      ...
      >
        <div>
          ...
          <h2 style={{ margin: "0 0 14px", fontSize: 26, lineHeight: 1.2 }}>
            Restaurateurs, épiceries, traiteurs ?
          </h2>
          <p
            style={{
              fontSize: 14,
          ...
          >
            Tarifs professionnels adaptés à vos volumes : livraisons régulières,
            commandes en gros, approvisionnement sur-mesure.
          </p>
          <div
            style={{
              display: "grid",
        ...
        >
          <div style={{ fontSize: 14, fontWeight: "bold", marginBottom: 14 }}>
            Nos clients professionnels :
          </div>
          {[
            "🍽️ Restaurants africains & caribéens",
            "🛒 Épiceries exotiques",
            "👨‍🍳 Traiteurs & cuisiniers à domicile",
            "🏫 Associations & cantines communautaires",
          ].map((item) => (
            <div
              key={item}
              style={{
  ...
  return (
    <div style={card}>
      <SectionTitle
        ...
        subtitle="Choisissez votre mode de paiement à l'étape de confirmation."
      />
      <div
        style={{
          display: "grid",
        ...
        {[
          {
            icon: "🅿️",
            label: "PayPal",
            desc: "Lien automatique — montant pré-rempli",
            color: "#003087",
            bg: "#e8f0fe",
          },
          {
            icon: "💜",
            label: "Wero",
            desc: "Numéro en surbrillance à copier",
            color: "#5b21b6",
            bg: "#f5f3ff",
          },
          {
            icon: "🏦",
            label: "Virement",
            desc: "Coordonnées bancaires SEPA",
            color: T.green,
            bg: T.greenLight,
          },
        ].map((m) => (
          ...
          >
            ...
            <div style={{ fontWeight: "bold", color: m.color, fontSize: 15 }}>
              {m.label}
            </div>
      ...
      >
        🔒 Les détails de paiement apparaissent sur la page de confirmation,
  ...
  return (
    <>
        ...
        >
          <div>
            ...
            <p style={{ margin: 0, opacity: 0.85, fontSize: 12 }}>
              Légumes africains BIO cultivés en France 🇫🇷
            </p>
          </div>
      ...
      <Section>
        <div
          style={{
            background: `linear-gradient(135deg, ${T.green}, #2f9e44)`,
            color: T.white,
            borderRadius: 22,
            padding: "32px 28px",
        ...
        >
          ...
          >
            <div>
              ...
              >
                Frais, naturels, sans pesticides. Du champ à votre assiette,
                sans intermédiaire.
              </p>
              <div
                style={{
              ...
              >
                <div>
                  👉 Livraison Relais offerte dès {FREE_DELIVERY_THRESHOLD} €
                </div>
        ...
        <div className="col">
          <Section id="packs">
            <div style={card}>
              <SectionTitle
                emoji="🧺"
                title="Nos packs"
                subtitle="Les offres les plus demandées, prêtes à commander."
              />
              <div
                style={{
                  display: "grid",
          ...
          <Section bg={T.bgAlt}>
            <div style={card}>
              <SectionTitle
                emoji="🥬"
                title="Produits à l'unité"
                subtitle="Par tranche de 0,5 kg — récolte hebdomadaire, livraison fraîche."
                action={
                  <span
                    style={{
                      background: T.amberLight,
              ...
              />
              {cart.totalUnitWeight > 0 && (
                <BulkBanner
                  totalUnitWeight={cart.totalUnitWeight}
          ...
          <Section>
            <div style={card}>
              <SectionTitle
                ...
                subtitle="Via Mondial Relay — calculés selon le poids réel de votre commande."
              />
              <div
                style={{
                  display: "grid",
              ...
              >
                ...
                >
                  ...
                  >
                    🎁 Offerte dès {FREE_DELIVERY_THRESHOLD} € ou avec le Pack
                    Premium
                  </div>
                </div>
                <div
                ...
                >
                  ...
                  <p style={{ color: T.gray, fontSize: 14, marginBottom: 10 }}>
                    Tarif Point Relais +{" "}
                    <strong>{MR_HOME_SURCHARGE.toFixed(2)} €</strong> de
                    supplément.
                  </p>
                  <div
                    style={{
                  ...
                  >
                    ⚠️ Toujours payante, même au-delà de{" "}
          ...
          <Section>
            <div style={card}>
              ...
              >
                <div>
                  <p
                    style={{ color: T.gray, lineHeight: 1.7, marginBottom: 14 }}
                  >
                    Nous avons créé <strong>La Serre Africaine</strong> avec une
                    vision simple : reconnecter la diaspora à ses racines,
                    proposer une agriculture saine et valoriser les saveurs
              ...
              >
                {[
                  ...
                  <ValidatedInput
                    ...
                    onChange={(v) => updateCustomer(field, v)}
                    validate={validate}
                  />
                ))}
                <PhoneInput
        ...
        <div>
          ...
          {cart.subtotal > 0 && (
            <div style={{ fontSize: 12, opacity: 0.85 }}>
              {cart.grandTotal.toFixed(2)} € total
            </div>
          )}
        </div>
        <a
      ...
      <section
        style={{ maxWidth: 1380, margin: "0 auto", padding: "0 16px 60px" }}
      >
        <div
          style={{
            background: `linear-gradient(135deg, ${T.green}, #2f9e44)`,
        ...
        >
          <h2 style={{ marginTop: 0, fontSize: 28, fontWeight: "bold" }}>
          ...
          >
            Recevez vos légumes frais chez vous, prêts à cuisiner.
          </p>
          <a
            href="#commande"
            style={{
      ...
      >
        Merci de faire grandir <strong>La Serre Africaine</strong>.
        &nbsp;|&nbsp; Pro : {PRO_PHONE}
      </footer>
    </>
  );
